...
class Validator {
  ...
  getRequiredErrors(schema, body, schemaKey) {
    let requiredKey = schema[schemaKey].required;
    let errors = [];
    ...
    if (requiredKey) {
      ...
      } else if (typeof body[schemaKey] === 'undefined' ||
      body[schemaKey] === null) {
        errors.push(schemaKey + ERRORS.REQUIRED_PARAM);
      }
    }
  ...
  getTypeErrors(schema, body, schemaKey) {
    let typeKey = schema[schemaKey].type.name;
    let errors = [];
  ...
  validateJSONSchema(schema, body, top) {
    ...
    schemaKeys.forEach((schemaKey) => {
      if (this.isNotConstraintKey(schemaKey)) {
        ...
        if (isRequired) {
          errors = errors
            .concat(this.getRequiredErrors(schema, body, schemaKey));
        }
    ...
    this.spliceConstraintKeys(schemaKeys).forEach((schemaKey) => {
      ...
      if (body && typeof body[schemaKey] === 'object') {
        errors = errors
          .concat(this.validateJSONSchema(schema[schemaKey],
            body[schemaKey], top));
      }
    });
  ...
  isValidJSONArraySchema(schema, alertArray) {
    return new Promise((res, rej) => {
      ...
      const asyncCallbackHandler = (brutalException, resultArray) => {
        ...
        let finalResultArray = resultArray.map(result => {
        ...
        return res({
          ...
          success: finalResultArray
            .filter(el => el !== null).map(el => el.value),
        });
      };
